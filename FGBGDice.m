...
function score = FGBGDice(inLabel,gtLabel)
% inLabel: label image to be evaluated. Background label is assumed to be the lowest one.
% gtLabel: ground truth label image. Background label is assumed to be the lowest one.
% score: Dice score for foreground/background segmentation, only.
...
% Dice score for the foreground
inMask = (inLabel>minInLabel); % find region of the foregound in inLabel
gtMask = (gtLabel>minGtLabel); % find region of the foregound in gtLabel
inSize = sum(inMask(:)); % cardinality of the found set in inLabel
gtSize = sum(gtMask(:)); % cardinality of the found set in gtLabel
overlap= sum(inMask(:).*gtMask(:)); % cardinality of overlap of the two regions
score = 2*overlap/(inSize + gtSize); % Dice score for the foreground
